...
# MAGIC %run ./BMI_gestation $weeks=15 $RPStartDate=$RPStartDate $RPEndDate=$RPEndDate $source=$source
...
# MAGIC  FROM BMI_Calc
# MAGIC  WHERE CCONTACTDATE <= Date_Xweek
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMPORARY VIEW BMI_Recorded_15week AS (
# MAGIC   SELECT * except(Date_Xweek)
# MAGIC   FROM BMI_Recorded
# MAGIC   WHERE CCONTACTDATE <= Date_Xweek
# MAGIC );
# MAGIC CREATE OR REPLACE TEMPORARY VIEW BMI_Diagnosed_15week AS (
# MAGIC   SELECT * except(Date_Xweek)
# MAGIC   FROM BMI_Diagnosed
# MAGIC   WHERE DIAGDATE <= Date_Xweek
# MAGIC );
...
# MAGIC  UNION 
# MAGIC  Select  MSD101_ID,'$RPStartDate' RPSTARTDATE, '$RPEndDate' RPENDDATE,PERSON_ID_MOTHER, UNIQPREGID, ORGCODEPROVIDER,BMI_RANGES, 'recorded_id' as statdetail from BMI_Recorded_15week
# MAGIC  UNION 
# MAGIC  Select  MSD101_ID,'$RPStartDate' RPSTARTDATE, '$RPEndDate' RPENDDATE,PERSON_ID_MOTHER, UNIQPREGID, ORGCODEPROVIDER,BMI_RANGES, 'diag_id' as statdetail from BMI_Diagnosed_15week
# MAGIC ;
...
# MAGIC SELECT * FROM UNIQPREG WHERE ORGCODEPROVIDER IN 
...
# MAGIC
# MAGIC select d.*,coalesce(n.BMI_Ranges,'BMI_NotRecorded') Ranges from uniqpreg d
...
dbutils.notebook.exit("Notebook: BMI_14Weeks ran successfully")