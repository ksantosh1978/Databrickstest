...
# MAGIC %md #THIS NOTEBOOK IS NO LONGER USED - SBL V3 ONWARDS

# COMMAND ----------

# MAGIC %md 
# MAGIC
# MAGIC ###Saving Babies Lives - Element 2 Outcome Indicator 2     
# MAGIC
# MAGIC Denominator: Singleton, liveborn babies born in the reporting month whose birth gestation is more than 39+6    
# MAGIC Numerator: Singleton, liveborn babies born in the reporting month whose birth gestation is more than 39+6 and their birthweight is under the 10th centile     
# MAGIC
...
# MAGIC  - Calculations of birth weight centile use the estimated fetal weight charts published (https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.1002220). This only considers baby gestation / sex in determining centiles for weight. Other more complicated models of baby / child centile exist (GROW etc). This set of data only contains values up to 40+6 weeks gestation. To avoid missing very low weight babies of 41-45 weeks gestation, this metric uses the 40 week limit for all births at 40-45 weeks. This will be an approximation for late term babies.    
# MAGIC
...
msd301 = table(dbSchema + ".msd301labourdelivery")
...
#Codes and units required for centile / weight
...
estimated_fetal_growth_10thtile_table11 = [(14,78), (15,99), (16, 124), (17, 155), (18, 192), (19, 235), (20, 286), (21, 345), (22, 412), (23, 489), (24, 576), (25, 673), (26, 780), (27, 898), (28, 1026), (29, 1165), (30, 1313), (31, 1470), (32, 1635), (33, 1807), (34, 1985), (35, 2167), (36, 2352), (37, 2537), (38, 2723), (39, 2905), (40, 3084)  ]

schema1 = StructType([ \
    StructField("GestationAge",IntegerType(),True), \
    StructField("TenthCentile",IntegerType(),True) \
  ])
 
efw_10 = spark.createDataFrame(data=estimated_fetal_growth_10thtile_table11,schema=schema1)

#Reference data above is for all babies. Publication also has sex specific estimates for centiles 5 to 95 (does not exist for lower centiles, so not relevant for E2_O1)
estimated_fetal_growth_10thtile_female_table14 = [(14,77), (15,97), (16, 122), (17, 152), (18, 188), (19, 231), (20, 281), (21, 339), (22, 405), (23, 481), (24, 567), (25, 663), (26, 769), (27, 886), (28, 1013), (29, 1150), (30, 1296), (31, 1451), (32, 1614), (33, 1783), (34, 1957), (35, 2135), (36, 2314), (37, 2493), (38, 2670), (39, 2843), (40, 3010)  ]

estimated_fetal_growth_10thtile_male_table15 = [(14,79), (15,100), (16, 127), (17, 158), (18, 196), (19, 241), (20, 293), (21, 354), (22, 424), (23, 503), (24, 592), (25, 692), (26, 803), (27, 924), (28, 1055), (29, 1197), (30, 1349), (31, 1509), (32, 1677), (33, 1852), (34, 2032), (35, 2217), (36, 2404), (37, 2591), (38, 2778), (39, 2962), (40, 3142)  ]

efw_10_f = spark.createDataFrame(data=estimated_fetal_growth_10thtile_female_table14,schema=schema1)
efw_10_m = spark.createDataFrame(data=estimated_fetal_growth_10thtile_male_table15,schema=schema1)
...
#Records from the MSD401/MSD301 tables
#msd401 = table("mat_pre_clear.msd401babydemographics")
#msd301 = table("mat_pre_clear.msd301labourdelivery")

#Birth in reporting period
...
#Livebirth
msd401 = msd401.filter(msd401.PregOutcome == '01')
#Born after 39+6 weeks gestation 
msd401 = msd401.filter(msd401.GestationLengthBirth > 279)

#Singleton
msd401 = msd301.join(msd401, ["Person_ID_Mother", "UniqPregID", "LabourDeliveryID", "RecordNumber"], "inner").select(msd401['*'],msd301['BirthsPerLabandDel'])
# msd401 = msd301.alias("a").join(msd401.alias("b"), ["Person_ID_Mother", "UniqPregID", "LabourDeliveryID", "RecordNumber"], "inner").select(["b.*","a.BirthsPerLabandDel"])
msd401 = msd401.filter(msd301.BirthsPerLabandDel == '1')

#Identify latest birth record if multiple exist for a baby
...
msd_denom = msd401.select(msd401.Person_ID_Baby, msd401.Person_ID_Mother, msd401.UniqPregID, msd401.OrgCodeProvider, msd401.OrgSiteIDActualDelivery, msd401.RecordNumber, msd401.MSD401_ID, msd401.PersonBirthDateBaby, msd401.GestationLengthBirth, msd401.PersonPhenSex).withColumnRenamed("OrgSiteIDActualDelivery", "DeliverySite").distinct()

#Final output dataframe for the denominator cohort must be set to have the name msd_denom
msd_denom = msd_denom.select(["Person_ID_Baby", "Person_ID_Mother", "UniqPregID", "OrgCodeProvider", "RecordNumber", "DeliverySite", "GestationLengthBirth", "PersonPhenSex"]).distinct()
...
#Centile is directly recorded by the provider
#msd405 = table("mat_pre_clear.msd405careactivitybaby")

#Only records for babies in the cohort (born in reporting month)
#Does not have do be under the same pregnancy / mother. Some babies appear under multiple pregnancy IDs
msd405 = msd405.alias("a").join(msd401.alias("b"), ["Person_ID_Baby"], "inner").select(["a.*", "b.PersonBirthDateBaby", "b.GestationLengthBirth", "b.PersonPhenSex"])
...
msd405_o = msd405.filter(msd405.RPStartDate <= StartDate).filter(msd405.ClinInterDateBaby >= msd405.PersonBirthDateBaby).filter(msd405.ClinInterDateBaby <= EndDate).filter((msd405.ObsCode.isin(Weight_Centile_Codes_Obs) == 'true')).\
            select(msd405.Person_ID_Baby, msd405.Person_ID_Mother, msd405.UniqPregID, msd405.PersonBirthDateBaby, msd405.PersonPhenSex, msd405.ClinInterDateBaby, msd405.GestationLengthBirth, msd405.ObsCode, msd405.ObsValue, msd405.UCUMUnit, msd405.RecordNumber, msd405.MSD405_ID, msd405.OrgCodeProvider)
...
#Flag records where centile is recorded on the birth date of the baby; how many days after centile was recorded; whether centile value is below 10
msd405_o = msd405_o.withColumn("OnDateFlag", when(msd405_o.ClinInterDateBaby == msd405_o.PersonBirthDateBaby, '1').otherwise("0")).withColumn("DateDifference", datediff(msd405_o.ClinInterDateBaby, msd405_o.PersonBirthDateBaby)).withColumn("UnderTen", when(msd405_o.ObsValue_Num < 10, "1").otherwise("0"))
...
msd_cent = msd405_o.select(msd405_o.Person_ID_Baby, msd405_o.UniqPregID, msd405_o.Person_ID_Mother, msd405_o.PersonBirthDateBaby, msd405_o.PersonPhenSex, msd405_o.GestationLengthBirth, msd405_o.ClinInterDateBaby, msd405_o.ObsCode, msd405_o.ObsValue_Num, msd405_o.UnderTen)
...
msd405_w = msd405.filter(msd405.RPStartDate <= StartDate).filter(msd405.ClinInterDateBaby >= msd405.PersonBirthDateBaby).filter(msd405.ClinInterDateBaby <= EndDate).filter((msd405.ObsCode.isin(Birth_Weight_Codes_Obs) == 'true')).\
            select(msd405.Person_ID_Baby, msd405.Person_ID_Mother, msd405.UniqPregID, msd405.PersonBirthDateBaby, msd405.PersonPhenSex, msd405.ClinInterDateBaby, msd405.ObsCode, msd405.ObsValue, msd405.UCUMUnit, msd405.RecordNumber, msd405.MSD405_ID, msd405.OrgCodeProvider, msd405.RPStartDate, msd405.GestationLengthBirth)
...
#Follow birthweight logic as in other MSDS measures - either birthweight specific weight code, or general weight code recorded on birth date
...
#Join to estimated fetal weight reference data (uses 40 for all valid gestations over that limit)
msd_help = msd_help.withColumn("GestLimitedAge", when(msd_help.GestationLengthBirth.between(280,315), lit(40)).otherwise(msd_help.GestWeeks))

msd_help = msd_help.alias("g").join(efw_10.alias("h"), efw_10.GestationAge == msd_help.GestLimitedAge, "left").drop(efw_10.GestationAge).withColumnRenamed("TenthCentile","TenthCentile_Gen")
msd_help = msd_help.alias("g").join(efw_10_f.alias("v"), efw_10_f.GestationAge == msd_help.GestLimitedAge, "left").drop(efw_10_f.GestationAge).withColumnRenamed("TenthCentile","TenthCentile_F")
msd_help = msd_help.alias("g").join(efw_10_m.alias("w"), efw_10_m.GestationAge == msd_help.GestLimitedAge, "left").drop(efw_10_m.GestationAge).withColumnRenamed("TenthCentile","TenthCentile_M")

#Add flag for whether a baby is under 10th centile, at or above 10th centile, not eligible (because there is a limited range of applicable gestations in the reference data)
msd_help = msd_help.withColumn("Under10Cent", when((msd_help.Weight_Grams < msd_help.TenthCentile_F) & (msd_help.PersonPhenSex == '2'), "1").\
                               otherwise(when((msd_help.Weight_Grams < msd_help.TenthCentile_M) & (msd_help.PersonPhenSex == '1'), "1").\
                                         otherwise(when((msd_help.Weight_Grams < msd_help.TenthCentile_Gen) & (msd_help.PersonPhenSex != '1') & (msd_help.PersonPhenSex != '2'), "1").\
                                                   otherwise(when(msd_help.GestLimitedAge.between(19,40) == 'true',"0").otherwise("99")))))

#Summarise weight data
msd_weighted = msd_help.select(msd_help.Person_ID_Baby, msd_help.UniqPregID, msd_help.Person_ID_Mother, msd_help.PersonBirthDateBaby, msd_help.PersonPhenSex, msd_help.ClinInterDateBaby, msd_help.GestationLengthBirth, msd_help.Weight_Grams, msd_help.Under10Cent).distinct() 



#From babies with centile (recorded or calculated)
#Which are under 10th centile
msd_under10_cent = msd_cent.filter(msd_cent.UnderTen == '1').select(msd_cent.Person_ID_Mother, msd_cent.Person_ID_Baby, msd_cent.UniqPregID)
msd_under10_weig = msd_weighted.filter(msd_weighted.Under10Cent == '1').select(msd_weighted.Person_ID_Mother, msd_weighted.Person_ID_Baby, msd_weighted.UniqPregID)
msd_under10 = msd_under10_cent.union(msd_under10_weig)


msd_num = msd_under10.select(msd_under10.Person_ID_Baby).distinct()
...
sble2_o2_spdf.write.format('delta').mode('overwrite').saveAsTable(f'{outSchema}.{outtable}')
