...
# %sql
# CREATE WIDGET TEXT RPStart DEFAULT "2021-06-01";
# CREATE WIDGET TEXT RPEnd DEFAULT "2021-06-30";
...
# MAGIC FROM global_temp.CoC_Measure_4_CoC_Denominator_Raw d
# MAGIC
...
# MAGIC %sql
# MAGIC  
# MAGIC CREATE OR REPLACE GLOBAL TEMP VIEW CoC_Measure_4_CoC_Numerator_Raw AS 
# MAGIC SELECT * 
# MAGIC FROM (SELECT d.KeyValue 
# MAGIC             , d.Person_ID, d.PregnancyID, d.OrgCodeProvider, d.AntenatalAppDate
# MAGIC             , 102cp.CarePlanDate, 102cp.RecordNumber, 102cp.ContCarePathInd
...
# MAGIC             , d.Indicator, d.IndicatorFamily
# MAGIC             , 0 AS OverDQThreshold
# MAGIC             , d.CreatedAt
# MAGIC             
# MAGIC       FROM global_temp.CoC_Measure_4_CoC_Denominator_Raw d
...
# MAGIC       WHERE 102cp.RPStartDate <= '$RPBegindate'
...
# MAGIC        , null as ethniccategory
# MAGIC        , null as ethnicgroup
# MAGIC FROM global_temp.CoC_Measure_4_CoC_Numerator_Raw d
# MAGIC

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from $outSchema.CoC_Denominator_Geographies where indicator = 'COC_DQ04'
...
# MAGIC         WHERE Indicator = 'COC_DQ04'
...
# MAGIC %sql
# MAGIC  
# MAGIC UPDATE $outSchema.CoC_Provider_Aggregated SET OverDQThreshold = CASE WHEN Rounded_Rate > 5 THEN 1 ELSE 0 END WHERE Indicator = 'COC_DQ04'
# MAGIC
...
# MAGIC --       USING (select OrgCodeProvider, OverDQThreshold from {outSchema}.CoC_Provider_Aggregated WHERE Indicator = 'COC_DQ04' group by OrgCodeProvider, OverDQThreshold HAVING COUNT(*) = 1) as agg 
# MAGIC --       ON nr.OrgCodeProvider = agg.OrgCodeProvider AND nr.Indicator = 'COC_DQ04' 
...
# MAGIC --       USING (select OrgCodeProvider, OverDQThreshold from {outSchema}.CoC_Provider_Aggregated WHERE Indicator = 'COC_DQ04' group by OrgCodeProvider, OverDQThreshold HAVING COUNT(*) = 1) as agg
# MAGIC --       ON dr.OrgCodeProvider = agg.OrgCodeProvider AND dr.Indicator = 'COC_DQ04'
# MAGIC --       WHEN MATCHED THEN UPDATE SET dr.OverDQThreshold = agg.OverDQThreshold
# MAGIC       

# COMMAND ----------

dbutils.notebook.exit("Notebook: COCDQ04 ran successfully")