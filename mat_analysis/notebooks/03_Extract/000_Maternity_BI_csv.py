...
# dbutils.widgets.text("RPBegindate","2020-09-01")
# dbutils.widgets.text("RPEnddate", "2020-09-30")
...
# %run /data_analysts_collaboration/CMH_TURING_Team(Create)/code_sharing/geogtlrr_table_creation_new $endperiod=$RPEnddate $outSchema=$userDB
...
msd101['AgeAtBookingMotherGroup']= np.where(msd101['AgeAtBookingMother'].isna(), 'Missing Value', 
...
msd101['ComplexSocialFactorsInd']= np.where(msd101['ComplexSocialFactorsInd'].isna(),'Missing',
...
msd001['DeprivationDecileAtBooking'] = np.where(msd001['Rank_IMD_Decile_2015'].notna(),
										...
										np.where((msd001['Rank_IMD_Decile_2015'].isna()) & (msd001['LSOAMother2011'].notna()), 'Resident Elsewhere in UK',
										np.where((msd001['Rank_IMD_Decile_2015'].isna()) & (msd001['PostcodeDistrictMother'].notna()), 'Pseudo Postcode','Value outside reporting parameters')))
...
msd101['PreviousCaesareanSectionsGroup'] = np.where(msd101['PreviousCaesareanSections'] >= 1, 'At least one Caesarean Section',
                                                   ...
                                                   np.where(
                                                     ((msd101['PreviousCaesareanSections'].isna()) | (msd101['PreviousCaesareanSections'] == 0)) &
                                                     ((msd101['PreviousLiveBirths'].isna()) | (msd101['PreviousLiveBirths'] == 0)) &
                                                     ((msd101['PreviousStillBirths'].isna()) | (msd101['PreviousStillBirths'] == 0)),
...
msd_sk2Sk = msd401[['Person_ID_Baby', 'OrgCodeProvider', 'RPStartDate', 'SkintoSkin']].copy()
...
print(f'Some organisations does not have an Mbrrace_Grouping: {check_pdf["OrgCodeProvider"].unique()}')
# orgs without mbrrace group are mapped to `Group 6. Unknown`
msd_concat['Mbrrace_Grouping'] = msd_concat['Mbrrace_Grouping'].fillna('Group 6. Unknown')
msd_concat['Mbrrace_Grouping_Short'] = msd_concat['Mbrrace_Grouping_Short'].fillna('Group 6. Unknown')
